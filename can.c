...
can_mot_write(s32 axis, s32 value)
{
    ...
    // pick frame data bytes out of rob->can.value
    for (i = 0; i < frame.can_dlc; i++) {
        frame.data[i] = (rob->can.value >> (8 * i)) & 0xff;
    }
...
can_check_emergency(void) {
    ...
    if ((frame.can_id & 0xfff0) == 0x080) {
        ...
        char failmsg[512];
        sprintf(failmsg, "Received an emergency message from motor (%#02x). Shutting down motor power.\\n"
